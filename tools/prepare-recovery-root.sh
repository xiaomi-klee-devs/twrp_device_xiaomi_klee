...
#   2. keep only the touch/haptic/misc .ko modules chagall actually needs
    ...
    find "$ramdisk/lib/modules" -maxdepth 1 -type f -name '*.ko' -print0 |
        while IFS= read -r -d '' module; do
            case "$(basename "$module")" in
                nt38771_touch_chagall.ko|xiaomi_touch_chagall.ko|nxp_i2c.ko|p73.ko|spi-mt65xx.ko|irq-dbg.ko|haptic.ko|miev.ko|aac_haptic.ko)
                    ;;
                *)
                    rm -f "$module"
                    ;;
...
upx_binaries=(
    ...
    ozip_decrypt pigz reboot recovery resetprop resize2fs chagall_omapi_bridge
